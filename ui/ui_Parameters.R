tabPanel("Parameters", value = "Para",
  ...
  fluidRow(
    ...
    column(2,
      selectInput("distPar", h4("2. Select Distribution of First Stage
                                processing Time"),
    ...
    h4(
      tags$div(class = "help-tip",
        p("In FAP, the visual stimulus is set as the target stimulus. In RTP,
          the stimulus detected first functions as the target, independent from
          its modality.")),
      "3. Select Parameter values for the first stage (in ms)"),
    column(8,
  ...
  fluidRow(
    column(4,
      plotOutput("stage1_density", height = "auto")),
    column(4,
      plotOutput("prob", height = "auto")),
    column(4,
      tags$div(class = "help-tip",
        p("- A decrease in RTs in the bimodal task condition compared to the
          unimodal condition implies facilitation.", br(),
          "- Negative reaction times may be simulated due to unrealistic
          parameter settings.")),
      plotOutput("data", height = "auto")))
)
